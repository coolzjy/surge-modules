// https://raw.githubusercontent.com/jcsirot/digest.js/master/src/digest.js
/*! ***** BEGIN LICENSE BLOCK *****
 *!
...
(function (global) {
  ...
  md5Engine.prototype.processBlock = function (input) {
    ...
    var W5 =
      (input[23] << 24) | (input[22] << 16) | (input[21] << 8) | input[20];
    var W6 =
      (input[27] << 24) | (input[26] << 16) | (input[25] << 8) | input[24];
    var W7 =
      (input[31] << 24) | (input[30] << 16) | (input[29] << 8) | input[28];
    var W8 =
      (input[35] << 24) | (input[34] << 16) | (input[33] << 8) | input[32];
    var W9 =
      (input[39] << 24) | (input[38] << 16) | (input[37] << 8) | input[36];
    var Wa =
      (input[43] << 24) | (input[42] << 16) | (input[41] << 8) | input[40];
    var Wb =
      (input[47] << 24) | (input[46] << 16) | (input[45] << 8) | input[44];
    var Wc =
      (input[51] << 24) | (input[50] << 16) | (input[49] << 8) | input[48];
    var Wd =
      (input[55] << 24) | (input[54] << 16) | (input[53] << 8) | input[52];
    var We =
      (input[59] << 24) | (input[58] << 16) | (input[57] << 8) | input[56];
    var Wf =
      (input[63] << 24) | (input[62] << 16) | (input[61] << 8) | input[60];
  ...
  var pbkdf1 = function (digest, iterationCount) {
    ...
    return {
      deriveKey: function (password, salt, len) {
        return derive(
          convertToUint8Array(password),
          convertToUint8Array(salt),
          len
        );
      },
    };
  ...
  var pbkdf2 = function (prf, iterationCount) {
    ...
    var derive = function (password, salt, len) {
      ...
      for (i = 1; i <= l; i++) {
        tmpBuf.set(
          _F(password, salt, iterationCount, i),
          prf.hmacLength() * (i - 1)
        );
      }
...
function HMAC_MD5(payload, secret) {
  const mac = new Digest.HMAC_MD5();
  mac.setKey(secret);
  return new Uint8Array(mac.mac(payload)).reduce((acc, curr) => {
    return acc + (curr >> 4).toString(16) + (curr % 16).toString(16);
  }, "");
}

const UPPER_CASE_LETTER = "abeklnosuvwxy01279";

function fp(password, identifier) {
  const temp = HMAC_MD5(password, identifier);
  const source = HMAC_MD5(temp, "snow");
  const rule = HMAC_MD5(temp, "kise");
  ...
  return isNaN(code[0]) ? code.slice(0, 16) : "K" + code.slice(1, 16);
}
...
$done({
  response: {
    body: fp(params.password, params.identifier),
  },
});
